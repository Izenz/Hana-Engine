...
Texture ModuleTexture::Load(const char* texturePath) {
	...
	if (texId != 0) {
		...
		glTexImage2D(	GL_TEXTURE_2D, 0, GL_RGBA, texture.width = ilGetInteger(IL_IMAGE_WIDTH),
						texture.height = ilGetInteger(IL_IMAGE_HEIGHT), 0, GL_RGBA, GL_UNSIGNED_BYTE, ilGetData());
